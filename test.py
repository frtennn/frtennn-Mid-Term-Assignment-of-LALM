...
def evaluate_model(model: Transformer, test_loader, device: torch.device, criterion):
    """评估模型在测试集上的表现"""
    model.eval()
    total_loss = 0
    total_acc = 0
    ...
    with torch.no_grad():
        for src, tgt in tqdm(test_loader, desc="测试中"):
    ...
    # 修复：基于token的困惑度计算
    # 我们需要重新计算每个样本的损失来获得正确的总负对数似然
    if total_tokens > 0:
        # 由于我们使用的是平均损失，需要乘以token数来估计总负对数似然
        # 但更准确的方法是在每个batch中累加实际的损失值
        token_based_perplexity = math.exp(avg_loss)  # 在这种情况下，两种方法应该相同
    else:
        token_based_perplexity = perplexity
...
def show_example(model: Transformer, test_loader, device: torch.device, sp_processor, example_index: int = 0):
    """显示测试集中的一个例子及其翻译结果"""
    model.eval()
...
def main():
    parser = argparse.ArgumentParser(description='测试最佳Transformer模型')
    parser.add_argument('--config', type=str, default='configs/base.yaml', help='配置文件路径')
    parser.add_argument('--checkpoint', type=str, default='checkpoints/best_model.pt', help='检查点文件路径')
    ...
    # 修复：使用weights_only=True避免警告
    checkpoint = torch.load(args.checkpoint, map_location=device, weights_only=False)
    model.load_state_dict(checkpoint['model_state_dict'])
    print(f"成功加载检查点，来自第 {checkpoint['epoch']} 轮，验证损失: {checkpoint['loss']:.4f}")
